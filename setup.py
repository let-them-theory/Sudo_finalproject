...
def _recursive_data_files(src_dir):
    """src_dir 이하 모든 파일을 share/패키지/<같은 구조>로 설치하는 data_files 엔트리."""
    abs_src = os.path.join(_HERE, src_dir)
    entries = []
    for root, _dirs, files in os.walk(abs_src):
        if not files:
            continue
        rel_root = os.path.relpath(root, _HERE)
        dest = os.path.join('share', package_name, rel_root)
        sources = [os.path.join(rel_root, f) for f in files]
        entries.append((dest, sources))
    return entries


def _frontend_dist_data_files():
    """web_kiosk/frontend/dist 디렉터리를 glob 기반으로 설치 (colcon 호환)."""
    dist_dir = os.path.join(_HERE, 'web_kiosk', 'frontend', 'dist')
    if not os.path.isdir(dist_dir):
        return []
    entries = []
    share_base = os.path.join('share', package_name)
    for root, _dirs, files in os.walk(dist_dir):
        if not files:
            continue
        rel_root = os.path.relpath(root, _HERE)
        dest = os.path.join(share_base, rel_root)
        sources = glob(os.path.join(rel_root, '*'))
